...
=======
  ...
  //The thrust generated by the blade is in the square value
  //of the motor rotation speed. There is no linear dependancy
  //between ESC values and the thrust, this is qn qttempt to
  //acount for it.

  // int signe[DIM];
  // for (int i=0;i<DIM;i++){
  //   signe[i] = PIDcorrections[i]/fabs(PIDcorrections[i]);
  //   PIDcorrections[i] = fabs(PIDcorrections[i]);
  // }

  // servoval[0] =(int)(throttle - signe[PITCH]*sqrt(PIDcorrections[PITCH]));
  // servoval[1] =(int)(throttle + signe[PITCH]*sqrt(PIDcorrections[PITCH]));
  // servoval[2] =(int)(throttle + signe[ROLL]*sqrt(PIDcorrections[ROLL]));
  // servoval[3] =(int)(throttle - signe[ROLL]*sqrt(PIDcorrections[ROLL]));
